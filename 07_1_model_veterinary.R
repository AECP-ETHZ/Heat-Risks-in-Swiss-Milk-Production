...
#
# Part 7.1/9: Model for veterinary expenses (figure 3)
...
for (z in 1:length(names(sub_panels_ID))){
  ...
  sub_dairy_farms_panel_final <- dairy_farms_panel_final[which(dairy_farms_panel_final$farm %in%  sub_panels_ID[[z]] & dairy_farms_panel_final$sk_Tierarzt > 0),]
  ...
  lower_bound <- floor(quantile(sub_farm_THI_hourly$THI,0.05, type=1, na.rm=T))
  upper_bound <- ceiling(quantile(sub_farm_THI_hourly$THI,0.95, type=1, na.rm=T))
  ...
  temp_THI<-sub_farm_THI_hourly %>%
    group_by(farm,year)%>%
    summarise_at(vars(THI),sum, na.rm=T)
  ...
  temp_list_models[[1]] <- feols(log(sk_Tierarzt) ~ THI + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  rm(sub_farm_panel_THI_PP_yearly)
  ...
  for (k in 1:length(knot_range)){
    ...
    temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots= knot_range[k]))
    ...
    temp1 <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
    temp_RSS_1_knot[k] <- sum(resid(temp1)^2)
    rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly,temp1)
  ...
  temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots=  temp_list_knots[[1]]))
  sub_farm_THI_hourly_extended <- bind_cols(sub_farm_THI_hourly, temp_new_ts_THI_ls)
  colnames(sub_farm_THI_hourly_extended)[1] <- "farm"
  colnames(sub_farm_THI_hourly_extended)[8] <- "s_THI_1"
  colnames(sub_farm_THI_hourly_extended)[9] <- "s_THI_2"
  ...
  temp_list_models[[2]] <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly,temp1)
  ...
  for (k in 1:ncol(knots_2_combi_good)){
    ...
    temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots= knots_2_combi_good[,k]))
    sub_farm_THI_hourly_extended <- bind_cols(sub_farm_THI_hourly, temp_new_ts_THI_ls)
    colnames(sub_farm_THI_hourly_extended)[1] <- "farm"
    colnames(sub_farm_THI_hourly_extended)[8] <- "s_THI_1"
    colnames(sub_farm_THI_hourly_extended)[9] <- "s_THI_2"
    ...
    temp1 <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  ...
  temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots=  temp_list_knots[[2]]))
  sub_farm_THI_hourly_extended <- bind_cols(sub_farm_THI_hourly, temp_new_ts_THI_ls)
  colnames(sub_farm_THI_hourly_extended)[1] <- "farm"
  colnames(sub_farm_THI_hourly_extended)[8] <- "s_THI_1"
  colnames(sub_farm_THI_hourly_extended)[9] <- "s_THI_2"
  ...
  temp_list_models[[3]] <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly)
  ...
  for (k in 1:ncol(knots_3_combi_good)){
    ...
    temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots= knots_3_combi_good[,k]))
    sub_farm_THI_hourly_extended <- bind_cols(sub_farm_THI_hourly, temp_new_ts_THI_ls)
    colnames(sub_farm_THI_hourly_extended)[1] <- "farm"
    colnames(sub_farm_THI_hourly_extended)[8] <- "s_THI_1"
    colnames(sub_farm_THI_hourly_extended)[9] <- "s_THI_2"
    ...
    temp1 <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 +s_THI_4 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  ...
  temp_new_ts_THI_ls <- as.data.frame(lspline(sub_farm_THI_hourly$THI,knots=  temp_list_knots[[3]]))
  sub_farm_THI_hourly_extended <- bind_cols(sub_farm_THI_hourly, temp_new_ts_THI_ls)
  colnames(sub_farm_THI_hourly_extended)[1] <- "farm"
  colnames(sub_farm_THI_hourly_extended)[8] <- "s_THI_1"
  colnames(sub_farm_THI_hourly_extended)[9] <- "s_THI_2"
  ...
  temp_list_models[[4]] <- feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 +s_THI_4 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
  rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly)
  ...
  if( best_model_number == 1){
    ...
    list_final_model[[z]] <- feols(log(sk_Tierarzt) ~ THI + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
    rm(sub_farm_panel_THI_PP_yearly)
    ...
    list_final_model[[z]] <-  feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
    rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly) 
    ...
    list_final_model[[z]] <-  feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
    rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly) 
    ...
    list_final_model[[z]] <-  feols(log(sk_Tierarzt) ~ s_THI_1 + s_THI_2+ s_THI_3 + s_THI_4 + precip +pp2 | farm + year, data =sub_farm_panel_THI_PP_yearly,  cluster = c("farm","year"))
    rm(temp_new_ts_THI_ls, sub_farm_THI_hourly_extended, sub_farm_panel_THI_PP_yearly) 
  }
  
  # Tidy up environment
  rm(best_model_number, temp_list_models,temp_list_knots,sub_farm_THI_hourly, sub_farm_precip_daily , sub_dairy_farms_panel_final, lower_bound, upper_bound, knot_range, best_model_number,  temp_list_models)
  rm(differences,knots_2_combi, knots_2_combi_good, knots_3_combi, knots_3_combi_good, temp_CP, sub_farm_precip_daily, sub_farm_THI_hourly, sub_dairy_farms_panel_final)
  
}

list_final_model_vet <- list_final_model
list_final_knot_combination_vet <- list_final_knot_combination


save(list_final_model_vet, file="Models/final_model_vet.RData")
save(list_final_knot_combination_vet, file="Models/final_knots_vet.RData")