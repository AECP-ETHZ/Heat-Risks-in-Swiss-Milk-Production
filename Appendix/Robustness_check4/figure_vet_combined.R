...
#
# Robustness check 4: figure for veterinary expenses
#
# ==================================================================================================================================
# ==================================================================================================================================
...
load("APPENDIX/Milchleistung/final_knots_vet_highmilk.RData")
load("APPENDIX/Milchleistung/final_model_vet_highmilk.RData")

list_final_model_vet_up <- list_final_model_vet
list_final_knot_combination_vet_up <- list_final_knot_combination_vet
rm(list_final_model_vet, list_final_knot_combination_vet)

# Load farms with low milk production
load("APPENDIX/Milchleistung/final_knots_vet_milkdown.RData")
load("APPENDIX/Milchleistung/final_model_vet_milkdown.RData")

list_final_model_vet_down <- list_final_model_vet
list_final_knot_combination_vet_down <- list_final_knot_combination_vet
...
THI_range <- seq(ceiling(min(farm_THI_hourly$THI, na.rm=T)), floor(max(farm_THI_hourly$THI, na.rm=T)),0.0025)
...
for (s in 1:length(plot_titles)){
  ...
  # Data preparation
  temp_model_up <- list_final_model_vet_up[[s]]
  temp_knot_up  <- list_final_knot_combination_vet_up[[s]]
  
  sub_dairy_farms_panel_final_up <- dairy_farms_panel_final[which(dairy_farms_panel_final$farm %in% sub_panels_ID_up[[s]] & dairy_farms_panel_final$sk_Tierarzt > 0),]
  ...
  temp_model_down <- list_final_model_vet_down[[s]]
  temp_knot_down  <- list_final_knot_combination_vet_down[[s]]
  ...
  list_centered_plots[[s]] <- ggplot()+ggtitle(paste(plot_titles[s],sep=""))+
    ...
    geom_rect(aes(xmin=8, xmax=90, ymin=0.0041, ymax=0.0065), color="white",fill="white")+
    
    annotate("text", x = 55, y = 0.0060, label = paste("high milk yield subsample (",number_farms_up," farms)",sep=""), colour = "brown1",family="Times New Roman",size=2)+
    annotate("text", x = 54, y = 0.0050, label = paste("low milk yield subsample (",number_farms_down," farms)",sep=""), colour = "cornflowerblue",family="Times New Roman",size=2)+
...
ggsave(plot=plot_vet_centered,"APPENDIX/Milchleistung/vet_samples.pdf", width = 16, height = 16, unit="cm", device = cairo_pdf)
ggsave(plot=plot_vet_centered,"APPENDIX/Milchleistung/vet_samples.png", width = 16, height = 9, unit="cm")
