...
#
# Robustness check 4: figure for feed purchases
#
# ==================================================================================================================================
# ==================================================================================================================================
...
load("APPENDIX/Milchleistung/final_knots_feed_highmilk.RData")
load("APPENDIX/Milchleistung/final_model_feed_highmilk.RData")
...
# Correction (wrong knot location)
list_final_knot_combination_feed_up[[3]] <- NA

# Load farms with low milk production
load("APPENDIX/Milchleistung/final_knots_feed__milkdown.RData")
load("APPENDIX/Milchleistung/final_model_feed_milkdown.RData")

list_final_model_feed_down <- list_final_model_feed
list_final_knot_combination_feed_down <- list_final_knot_combination_feed

# Correction (wrong knot location)
list_final_knot_combination_feed_down[[2]] <- NA
...
for (s in 1:length(plot_titles)){
  ...
  list_centered_plots[[s]] <- ggplot()+ggtitle(paste(plot_titles[s],sep=""))+
    ...
    annotate("text", x = 15, y = 0.0060, label = paste("high milk yield subsample (",number_farms_up," farms)",sep=""), colour = "brown1",family="Times New Roman",size=2)+
    annotate("text", x = 14.5, y = 0.0050, label = paste("low milk yield subsample (",number_farms_down," farms)",sep=""), colour = "cornflowerblue",family="Times New Roman",size=2)+
...
ggsave(plot=plot_feed_centered,"APPENDIX/Milchleistung/feed_samples.pdf", width = 16, height = 16, unit="cm", device = cairo_pdf)
ggsave(plot=plot_feed_centered,"APPENDIX/Milchleistung/feed_samples.png", width = 16, height = 9, unit="cm")
