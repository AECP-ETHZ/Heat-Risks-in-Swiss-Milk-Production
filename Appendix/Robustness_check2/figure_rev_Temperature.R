...
#
# Robustness check 2: figures for temperature effects on milk revenues
#
# ==================================================================================================================================
# ==================================================================================================================================
...
load("APPENDIX/TempOnly/final_knots_rev.RData")
load("APPENDIX/TempOnly/final_model_rev.RData")
...
for (s in 1:length(plot_titles)){
  ...
  list_absolut_plots[[s]] <- ggplot()+ggtitle(paste(plot_titles[s],"\n", "(",number_farms," farms",")",sep=""))+
    ...
    xlab("Temperature [°C]") + ylab("log(rev expenses) [CHF]")+
  ...
  list_centered_plots[[s]] <- ggplot()+ggtitle(paste(plot_titles[s],"\n", "(",number_farms," farms",")",sep=""))+
    ...
    xlab("Temperature [°C]") + ylab("log(milk revenues) [CHF]")+
...
ggsave(plot=plot_rev_centered,"Results(plot)/rev_main.pdf", width = 16, height = 16, unit="cm", device = cairo_pdf)
ggsave(plot=plot_rev_centered,"APPENDIX/TempOnly/rev_main.png", width = 16, height = 9, unit="cm")
